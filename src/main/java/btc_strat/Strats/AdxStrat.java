...
public class AdxStrat {
  ...
  public List<Float> canalAdxExitProfitList(List<Float> wList, int canalLookback, int adxLookback, Canal canal,
      int threshold, int exitThresh, int adxCount, List<Float> adxList, DI di, List<Candlestick> candleList) {
  ...
  public CanalAdxTrades canalAdxExitTrades(List<Float> wList, int canalLookback, int adxLookback, Canal canal,
      int threshold, int exitThres, int adxCount, List<Float> adxList, DI di, List<Candlestick> candleList) {
    List<Integer> startList = new ArrayList<>();
    startList.add(canalLookback + 1);
    startList.add(adxLookback + 1);
    startList.add((adxCount * 20) + 1);
  ...
  // cancal exit trades start
  public CanalAdxDetailsTrades canalAdxExitDetails(List<Float> wList, int canalLookback, int adxLookback, Canal canal,
      int threshold, int exitThres, int adxCount, List<Float> adxList, DI di, List<Candlestick> candleList) {
    List<Integer> startList = new ArrayList<>();
    startList.add(canalLookback + 1);
    startList.add(adxLookback + 1);
    ...
    CanalAdxDetailsTrades result = new CanalAdxDetailsTrades();
    float conf = stratUtils.calcConf(profitList);
    result.setExitThresh(exitThres);
    result.setCanalLookback(canalLookback);
    result.setLookback(adxLookback);
  ...
  public List<Float> canalAdxProfitList(int canalLookback, int adxLookback, Canal canal, int threshold, int adxCount,
      ...
      if (adxList.get(i) > threshold) {
        if (position == -1) {
          if (candleList.get(i).getClose() > canal.getHigherCanalList().get(i)
            ...
            position = 1;
            // profit = closeShortProfit(profit, open, candleList.get(i).getClose());
            // profitList.add(profit);
            open = candleList.get(i).getClose();
          }
        } else {
  ...
  // canal profit list finish

  // cancal trades start
  public CanalAdxTrades canalAdxTrades(int canalLookback, int adxLookback, Canal canal, int threshold, int adxCount,
      List<Float> adxList, DI di, List<Candlestick> candleList) {
    List<Integer> startList = new ArrayList<>();
    startList.add(canalLookback + 1);
    startList.add(adxLookback + 1);
      ...
      if (adxList.get(i) > threshold) {
        if (position == -1) {
          if (candleList.get(i).getClose() > canal.getHigherCanalList().get(i)
            ...
            // was short close it and go long
            position = 1;
            // profit = closeShortTrade(profit, open, candleList.get(i).getClose(),
  ...
  public List<Float> adxTrailingProfitConf(int kijunCount, Kijun kijun, int threshold, int lowerThreshold, int adxCount,
    ...
    for (int i = start; i < candleList.size(); i++) {
      if (adxList.get(i) > threshold) {
        ...
        if (kijun.getIsOverCloseList().get(i) && position == 1 && !di.getIsPositiveOver().get(i)
          ...
          if (!empty) {
            profit = stratUtils.closeLongExtraIterableProfit(maxPortions, profit, candleList.get(i).getClose(),
                positionList);
            profitList.add(profit);
          } else {
            empty = false;
        ...
        // short go long
            ...
            && isPrev1hTrailingLong(atrTrailingList1h, i, candleList.get(i).getClose())) {
          if (!empty) {
            profit = stratUtils.closeShortExtraIterableProfit(maxPortions, profit, candleList.get(i).getClose(),
                positionList);
            profitList.add(profit);
          } else {
            empty = false;
    ...
    if (!positionList.isEmpty()) {
      ...
      if (position == 1) {
        profit = stratUtils.closeLongExtraIterableProfit(maxPortions, profit,
            candleList.get(candleList.size() - 1).getClose(), positionList);
      } else {
        profit = stratUtils.closeShortExtraIterableProfit(maxPortions, profit,
            candleList.get(candleList.size() - 1).getClose(), positionList);
      }
      profitList.add(profit);
    }
  ...
  public TrailingAdxTrades adxTrailingTradesConf(int kijunCount, Kijun kijun, int threshold, int lowerThreshold,
      int adxCount, List<Float> adxList, DI di, int atrCount, List<Float> atrTrailingList1h,
      List<Candlestick> candleList, List<Candlestick> candleList1h) {
    int start = 0;
    if (kijunCount > ((adxCount * 20) + 1)) {
      start = kijunCount;
    } else if (atrCount * 2 > ((adxCount * 20) + 1)) {
    ...
    for (int i = start; i < candleList.size(); i++) {
      if (adxList.get(i) > threshold) {
        ...
        if (kijun.getIsOverCloseList().get(i) && position == 1 && !di.getIsPositiveOver().get(i)
            && !isPrev1hTrailingLong(atrTrailingList1h, i, candleList.get(i).getClose())) {
          if (!empty) {
            profit = stratUtils.closeLongExtraIterableTrades(maxPortions, profit, candleList.get(i).getClose(),
        ...
        // short go long
            ...
            && isPrev1hTrailingLong(atrTrailingList1h, i, candleList.get(i).getClose())) {
          if (!empty) {
            profit = stratUtils.closeShortExtraIterableTrades(maxPortions, profit, candleList.get(i).getClose(),
                tradePercentages, shortPercentages, positionList);
            profitList.add(profit);
          } else {
            empty = false;
          }
    ...
    TrailingAdxTrades result = new TrailingAdxTrades();
    float conf = stratUtils.calcConf(profitList);
    result.setConf(conf);
    result.setAdx(adxCount);
    result.setKijunLen(kijunCount);
    result.setExitThresh(lowerThreshold);
    result.setLongThresh(threshold);
    result.setLongPercentages(longPercentages);
    result.setShortPercentages(shortPercentages);
    result.setTradePercentages(tradePercentages);
  ...
  public KijunAdxTrades adxOnlyLongTradesConf(int kijunCount, Kijun kijun, int threshold, int closeLongThreshold,
    ...
    KijunAdxTrades result = new KijunAdxTrades();
    float conf = stratUtils.calcConf(profitList);
    result.setConf(conf);
    result.setAdx(adxCount);
    result.setKijunLen(kijunCount);
    result.setExitThresh(closeLongThreshold);
    result.setLongThresh(threshold);
    result.setLongPercentages(longPercentages);
    result.setTradePercentages(tradePercentages);
    result.setProfit(profit);
  ...
  public KijunAdxDetailsTrades adxOnlyLongDetails(int kijunCount, Kijun kijun, int threshold, int closeLongThreshold,
      int adxCount, List<Float> adxList, DI di, List<Candlestick> candleList) {
    int start = kijunCount > adxCount ? kijunCount * 10 : adxCount * 10;
    float profit = 1;
    boolean empty = true;
    ...
    KijunAdxDetailsTrades result = new KijunAdxDetailsTrades();
    float conf = stratUtils.calcConf(profitList);
    result.setConf(conf);
    result.setAdx(adxCount);
    result.setKijunLen(kijunCount);
  ...
  public KijunAdxTrades adxOnlyLongTrades(int kijunCount, Kijun kijun, int threshold, int closeLongThreshold,
      int adxCount, List<Float> adxList, DI di, List<Candlestick> candleList) {
    int start = kijunCount > adxCount ? kijunCount * 10 : adxCount * 10;
    float profit = 1;
    boolean empty = true;
  ...
  public KijunAdxTrades adxOnlyShortTrades(int kijunCount, Kijun kijun, int threshold, int closeShortThreshold,
      int adxCount, List<Float> adxList, DI di, List<Candlestick> candleList) {
    int start = kijunCount > adxCount ? kijunCount * 10 : adxCount * 10;
    float profit = 1;
    boolean empty = true;
    ...
    for (int i = start; i < candleList.size(); i++) {
      if (!position) {
        ...
        if (adxList.get(i) >= closeShortThreshold && di.getIsPositiveOver().get(i)
          ...
          // close long
          if (!empty) {
            profit = stratUtils.closeShortTrade(profit, open, candleList.get(i).getClose(), tradePercentages,
                shortPercentages);
          } else {
            empty = !empty;
          }
    ...
    if (position && !empty) {
      // should always be true
      profit = stratUtils.closeShortTrade(profit, open, candleList.get(candleList.size() - 1).getClose(),
          tradePercentages, shortPercentages);
    }
    KijunAdxTrades result = new KijunAdxTrades();
    result.setAdx(adxCount);
    result.setKijunLen(kijunCount);
    result.setExitThresh(closeShortThreshold);
    result.setLongThresh(threshold);
    result.setShortPercentages(shortPercentages);
    result.setTradePercentages(tradePercentages);
    result.setProfit(profit);
  ...
  public TrailingAdxTrades adxTrailingTrades(int kijunCount, Kijun kijun, int threshold, int lowerThreshold,
      int adxCount, List<Float> adxList, DI di, int atrCount, List<Float> atrTrailingList1h,
      List<Candlestick> candleList, List<Candlestick> candleList1h) {
    int start = 0;
    if (kijunCount > ((adxCount * 20) + 1)) {
  ...
  public TrailingAdxDetailsTrades adxTrailingTradesDetails(int kijunCount, Kijun kijun, int threshold,
      int lowerThreshold, int adxCount, List<Float> adxList, DI di, int atrCount, List<Float> atrTrailingList1h,
      List<Candlestick> candleList, List<Candlestick> candleList1h) {
    int start = 0;
    if (kijunCount > ((adxCount * 20) + 1)) {
      start = kijunCount;
    ...
    TrailingAdxDetailsTrades result = new TrailingAdxDetailsTrades();
    result.setAdx(adxCount);
    result.setKijunLen(kijunCount);
    result.setExitThresh(lowerThreshold);
    result.setLongThresh(threshold);
    ...
    result.setNegativeDetails(new5);
    return result;
  }
  ...
  public KijunAdxTrades adxThreshKijunProfit(int kijunCount, Kijun kijun, int threshold, int lowerThreshold,
      int adxCount, List<Float> adxList, DI di, List<Candlestick> candleList) {
    ...
    KijunAdxTrades res = new KijunAdxTrades();
  ...
  public KijunAdxTrades adxThreshKijunTrades(int kijunCount, Kijun kijun, int threshold, int lowerThreshold,
      int adxCount, List<Float> adxList, DI di, List<Candlestick> candleList) {
    // false = short | true = long
    int position = 1;
    int canOpenPosition = 0;
    ...
    if (!positionList.isEmpty()) {
      // should always be true
      if (position == 1) {
        profit = stratUtils.closeLongExtraIterableTrades(maxPortions, profit,
            candleList.get(candleList.size() - 1).getClose(), tradePercentages, longPercentages, positionList);
      } else {
        profit = stratUtils.closeShortExtraIterableTrades(maxPortions, profit,
            candleList.get(candleList.size() - 1).getClose(), tradePercentages, longPercentages, positionList);
      }
    }
    KijunAdxTrades result = new KijunAdxTrades();
