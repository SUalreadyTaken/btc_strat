...
public class MixUtil {
  ...
  public List<Float> getRsiList(float dayCount, List<Candlestick> candlestickList) {
    ...
    result.add(0F);
    // todo for 1h and 15min strategy.. need to make a list of cGainEma and cLossEma
    // for t-1
  ...
  public List<Double> getEmaList(int dayCount, List<Candlestick> candlestickList) {
    ...
    result.add(BigDecimal.valueOf((tmpMa / dayCount)).setScale(1, RoundingMode.HALF_UP).doubleValue());
    for (int i = dayCount; i < candlestickList.size(); i++) {
      double ema = BigDecimal.valueOf((candlestickList.get(i).getClose() * weight + result.get(i - 1) * (1 - weight)))
